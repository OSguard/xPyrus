...
class LatexRender {
    ...
    function getDimensions($filename) {
        ...
        $dim = array("x" => 1000, "y" => 1000);
        // CHANGE by linap, 27.06.2007
        // don't use $result[2] because it
        // returns wrong dimensions for empty images
        if (preg_match('/^(\d+)x(\d+)\D/', $result[3], $matches)) {
            $dim["x"] = $matches[1];
            $dim["y"] = $matches[2];
        }
